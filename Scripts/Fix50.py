# Made by LeoTorreZ,SilentNightSound and Gustav0
# Immense thanks to the Genshin Impact Modding Community for their help and support
# Specially,Zlevir and SinsofSeven for their aid on getting the resources and testing

# Fixes Bennett, Cyno, Fischl, Kirara, Mona/MonaCN, Nahida, Razor, Xinyan, Amber/AmberCN, Ningguang, Venti, Diluc and Xiao, Xianyun ,GaMing, Barbaba 3.6 and Layla
...
remaps = {"caddc4c6": ('Amber Global',[7,6,9,10,11,29,8,12,13,14,15,16,17,77,1,0,
          ...
          "7f5ef8cc": ('Xiao',[8, 47, 37, 7, 12, 13, 14, 10, 9, 11, 71, 48, 91, 38, 93,
          ...
          "fd47c861": ('Venti',[114, 115, 116, 91, 68, 49, 50, 47, 48, 51, 52, 53, 54,
                        ...
                        60, 61, 62,]),
          "02089582": ('Barbara',[54,12,8,9,77,17,18,19,13,14,10,11,15,16,53,100,101,102,56,49,7,6,79,51,20,75,98,55,78,1,2,0,3,4,5,38,37,30,
...
def collect_ini_files(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        if 'BufferValues' in root:
            continue
        for file in files:
            if file.lower().endswith('.ini') and not any(keyword in file.lower() for keyword in exclude_keywords):
            ...
            for x, vg_remap in remaps.items():
                if x in data:
                    same_folder_blends = [blend_file for blend_file in blend_files
                                          if os.path.dirname(os.path.join(folder_path, blend_file)) == folder_path]
                    if not same_folder_blends:
                        log_message(log, f'No blend file found for {vg_remap[0]} in folder: {folder_path}')
                        continue
                    ...
                    for blend_file in same_folder_blends:
                        try:
                            ...
                            create_backup(blend_file_path)  # Create a backup before remapping
...
if __name__ == '__main__':
    ...
    print(f"Total INI files found: {total_ini_files}")
    print(f"Processed {processed_files_count} INI files")
